...
USE_OPENVINO = True
USE_GPU = False  # Set to 'NPU' for Neural Processing Unit, True/'GPU' for GPU, False/'CPU' for CPU
...
def main():
    global CAMERA_INDEX
    global model, original_model, model_names, compiled_model, infer_request
    global inference_queue, latest_detections
    ...
    try:
        while True:
            ...
            if frame_count % NT_THROTTLE_FACTOR == 0:
                ...
                # Publish best pile target (for auto-collector)
                if pile_center is not None:
                    pub_pile_has_target.set(True)
                    pub_target_x.set(pile_center[0] * INCHES_TO_METERS)
